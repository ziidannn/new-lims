...
<div class="col-12 col-lg-12 order-2 order-md-3 order-lg-2 mb-4">
    ...
    @endif
    <form class="card" action="{{ route('result.ambient_air.add', $instituteSubject->id) }}" method="POST">
        @csrf
        <div class="col-xl-12">
            ...
            <div class="card-body">
                <div class="row">
                    <table class="table table-bordered">
                        ...
                        <tr>
                            <form class="card" action="{{ route('result.ambient_air.add', $instituteSubject->id) }}"
                                method="POST">
                                @csrf
                                ...
                                <td>
                                    ...
                                    @enderror"
                                    name="testing_result[{{ $parameter->id }}][]" value="{{ old("testing_result.{$parameter->id}.{$loop->index}",
                                            $resultData->isNotEmpty() ? $resultData->first()->testing_result : '') }}"
                                    required>
                                    @error("testing_result.{$parameter->id}.{$loop->index}")
...
<div class="col-12 col-lg-12 order-2 order-md-3 order-lg-2 mb-4">
    <form class="card" id="mainForm" action="{{ route('result.ambient_air.add', $instituteSubject->id) }}" method="POST">
        @csrf
        <div class="col-xl-12">
            <div class="card-body">
                <div class="row">
...
<script>
    ...
    swal({
        ...
        if (willSubmit) {
            ...
            // Sedikit delay agar user bisa melihat animasi loading
            setTimeout(() => {
