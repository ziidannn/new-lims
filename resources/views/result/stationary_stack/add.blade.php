...
@section('content')
<form class="card" id="main-analysis-form" action="{{ route('result.stationary_stack.add', $instituteSubject->id) }}" method="POST" onsubmit="return false;">
    @csrf
                    ...
                    @foreach ($parameters as $parameter)
                        ...
                        @else
                            ...
                            <tr>
                                <td>{{ $rowNumber++ }}</td>
                                <td class="text-start">{{ $parameter->name }}</td>
                                <td colspan="6" class="text-center text-muted">No parameter.</td>
                            </tr>
                        @endif
                    @endforeach
                </tbody>
